...
porém ele recebe um valor inteiro normal e não um endereço de memória o que provavelmente não será erro de compilação, 
já que existe espaço suficiente no endereço que o ponteiro p está para receber um valor inteiro, deste modo o motivo para advertência.

Na função printf, o compilador está recebendo dois ponteiros, o primeiro(p) que deveria mostrar o endereço para onde aponta
e o segundo(*p) que deveria mostrar o conteúdo deste endereço. Porém, o programa espera um endereço de memória como conteúdo. Assim, ao tentar 
acessar o conteúdo de um endereço de memória inesxistente, irá gerar um erro. Finalizando o programa inesperadamente sem a execução
*/
